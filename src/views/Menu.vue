        ...
        <div class="roll-machine">
          ...
          <div class="roll-window">
            ...
            <div class="roll-col roll-col-info">
              <div class="roll-col-label">类 别</div>
              <div class="roll-col-value">
                <span v-if="rollDisplayCat && rollDisplayCat !== '?'" :class="['roll-cat-circle', 'badge-' + rollDisplayCat]">{{ rollDisplayCat }}</span>
                <span v-else style="font-size:24px;color:#666;">?</span>
              </div>
              <div class="roll-col-divider"></div>
              <div class="roll-col-label">菜 名</div>
              <div class="roll-col-value roll-col-value-name">{{ rollDisplayName }}</div>
            </div>
            <!-- 右列：大图 -->
            <div class="roll-col roll-col-img-box">
              <div class="roll-col-label">今 日 推 荐</div>
              <div class="roll-col-value roll-col-img-area">
                <img v-if="rollDisplayImg" :src="rollDisplayImg" class="roll-result-img" loading="lazy" decoding="async" />
                <span v-else class="roll-img-placeholder">🍽️</span>
              </div>
            </div>
          </div>
          <div class="roll-machine-bottom"></div>
        </div>
...
.roll-section {
  background: #2a2a3e; border-radius: 20px; padding: 20px;
...
.roll-cat-label {
  display: flex; align-items: center; gap: 3px; font-size: 12px; cursor: pointer;
  padding: 3px 8px; border-radius: 5px;
}
.roll-cat-label input { width: 13px; height: 13px; }
.roll-cat-hint { font-size: 11px; color: #888; margin-left: 4px; }
.roll-cat-荤 { background: rgba(255,255,255,0.08); color: #f7c1c1; border: 1px solid rgba(255,255,255,0.1); }
.roll-cat-荤 input { accent-color: #a32d2d; }
.roll-cat-素 { background: rgba(255,255,255,0.08); color: #c0dd97; border: 1px solid rgba(255,255,255,0.1); }
.roll-cat-素 input { accent-color: #3b6d11; }
.roll-cat-汤 { background: rgba(255,255,255,0.08); color: #b5d4f4; border: 1px solid rgba(255,255,255,0.1); }
.roll-cat-汤 input { accent-color: #185fa5; }
.roll-cat-面 { background: rgba(255,255,255,0.08); color: #fac775; border: 1px solid rgba(255,255,255,0.1); }
.roll-cat-面 input { accent-color: #854f0b; }
.roll-cat-粉面 { background: rgba(255,255,255,0.08); color: #fac775; border: 1px solid rgba(255,255,255,0.1); }
.roll-cat-粉面 input { accent-color: #854f0b; }
.roll-cat-粉面类 { background: rgba(255,255,255,0.08); color: #fac775; border: 1px solid rgba(255,255,255,0.1); }
.roll-cat-粉面类 input { accent-color: #854f0b; }
.roll-cat-主 { background: rgba(255,255,255,0.08); color: #cecbf6; border: 1px solid rgba(255,255,255,0.1); }
.roll-cat-主 input { accent-color: #3c3489; }
.roll-cat-主食 { background: rgba(255,255,255,0.08); color: #cecbf6; border: 1px solid rgba(255,255,255,0.1); }
.roll-cat-主食 input { accent-color: #3c3489; }

/* 老虎机主体 */
.roll-machine {
  background: #1a1a30; border-radius: 12px; padding: 16px;
  margin-bottom: 14px; position: relative;
}
.roll-machine-title {
  text-align: center; font-size: 13px; font-weight: 600; color: #d4a843;
  letter-spacing: 6px; margin-bottom: 12px;
}
.roll-machine-top {
  width: 80%; height: 3px; margin: -8px auto 10px;
  background: #d4a843; border-radius: 2px; opacity: 0.6;
}
.roll-machine-bottom {
  width: 60%; height: 2px; margin: 10px auto -4px;
  background: #d4a843; border-radius: 1px; opacity: 0.3;
}

/* 页面标题 */
.roll-page-title { text-align: center; margin-bottom: 14px; }
.roll-page-title-main { font-size: 16px; font-weight: 600; color: #e0e0e0; margin-bottom: 4px; }
.roll-page-title-sub { font-size: 12px; color: #999; }

.roll-window {
  display: flex; gap: 8px; justify-content: center;
  min-height: 170px; height: 170px; align-items: stretch;
}
.roll-col { background: #0f0f1a; border-radius: 8px; overflow: hidden; border: 1px solid #333; display: flex; flex-direction: column; position: relative; height: 100%; }
.roll-col-info { flex: 0 0 90px; display: flex; flex-direction: column; }
.roll-col-img-box { flex: 1; min-width: 0; display: flex; flex-direction: column; position: relative; }
.roll-col-label {
  padding: 4px; font-size: 10px; color: #666; text-align: center;
  letter-spacing: 2px; background: #1a1a30;
}
.roll-col-divider { height: 1px; background: #333; margin: 0 12px; }
.roll-col-value { padding: 8px 6px; text-align: center; min-height: 36px; }
.roll-col-value-name { font-size: 14px; color: #e0e0e0; font-weight: 500; padding: 12px 6px; }
...
.roll-img-placeholder { font-size: 40px; opacity: 0.3; z-index: 1; }

/* 摇一下按钮 */
.roll-btn-row { text-align: center; }
.roll-btn-go {
  width: 100%; padding: 12px; background: #d4537e; color: #fff;
  border: none; border-radius: 20px; font-size: 14px; font-weight: 500;
  cursor: pointer; letter-spacing: 2px;
}
.roll-btn-go:disabled { opacity: 0.5; cursor: default; background: #666; }

/* 结果区 */
.roll-result {
  display: flex; align-items: center; gap: 6px;
  margin-top: 12px; padding: 8px 12px;
  border: 1px solid rgba(212,168,67,0.5); border-radius: 10px;
  background: rgba(212,168,67,0.08);
}
.roll-result-icon { font-size: 14px; }
.roll-result-text { font-size: 12px; color: #d4a843; font-weight: 600; }
.roll-cat-badge-sm {
  font-size: 10px; padding: 1px 6px; border-radius: 4px;
}
.roll-result-name { font-size: 13px; color: #e0e0e0; font-weight: 500; flex: 1; }
.roll-btn-add-sm {
  padding: 4px 12px; border-radius: 10px; font-size: 11px;
  background: transparent; color: #d4a843; border: 1px solid rgba(212,168,67,0.5);
  cursor: pointer; flex-shrink: 0;
}

.roll-count { text-align: center; font-size: 11px; color: #999; margin-top: 8px; }
.roll-added-msg { text-align: center; font-size: 12px; color: #a5d6a7; margin-top: 6px; }

/* 已选菜品列表（深色适配） */
.roll-cart-list { margin-top: 14px; border-top: 1px solid rgba(255,255,255,0.08); padding-top: 10px; }
.roll-cart-title { font-size: 12px; font-weight: 500; color: #ccc; margin-bottom: 6px; }
.roll-cart-item {
  display: flex; align-items: center; gap: 6px; padding: 5px 0;
}
.roll-cart-tag { font-size: 10px; padding: 1px 5px; border-radius: 3px; }
.roll-cart-name { font-size: 12px; color: #ccc; flex: 1; }
.roll-cart-qty { font-size: 11px; color: #888; }
.roll-cart-del { font-size: 12px; color: #666; cursor: pointer; padding: 0 4px; }
